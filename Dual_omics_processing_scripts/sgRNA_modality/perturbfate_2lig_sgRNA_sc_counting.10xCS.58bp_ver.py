...
###Written by Zihan Xu on 05/18/2022
...
def gRNA_cell_UMI_ident(sample, input_folder, inner_i7_bc_list, ligation_barcode_list1, \
                        ligation_barcode_list2, gRNA_correction_list, mismatch_rate = 1):
    ...
    while (line1):
        ...
        constant_R1 = line1[50:58].decode()
        constant_R2 = line2[10:20].decode()
        
        #use constant regions to filter pattern-matched reads and match i5 ligation barcodes
        if tmp_lig1 in ligation_barcode_list1 and tmp_lig2 in ligation_barcode_list2 and \
        constant_R1 in constant_mismatch_list1 \
        and constant_R2 in constant_mismatch_list2:
            #print("constant + lig passed")
            ligation_bc_match1 = ligation_barcode_list1[tmp_lig1]
            ligation_bc_match2 = ligation_barcode_list2[tmp_lig2]
            ...
            if inner_i7 in inner_i7_bc_list:
                ...
                gRNA_region = line2[34:54].decode()
                #match RT barcode and gRNA sequences
                if gRNA_region in gRNA_correction_list:
                    #print("CB and gRNA passed")
                    gRNA_match = gRNA_correction_list[gRNA_region]
                    filtered_line += 1
                    cell_id = sample + inner_i7_match + "." + ligation_bc_match1 + ligation_bc_match2
...
def gRNA_UMI_ident_parallel(input_folder, sampleID, output_folder, ligation_barcode_file1, \
                            inner_i7_bc_file, ligation_barcode_file2, gRNA_correction_file, \
    ...
    sample ID: %s
    output_folder: %s
    ligation barcode file1: %s
    ligation barcode file2: %s
    gRNA barcode correction file: %s
    inner i7 index barcode file: %s
    ___________________________________________________________________________
    ''' %(input_folder, sampleID, output_folder, ligation_barcode_file1, ligation_barcode_file2,\
    ...
    barcodes = open(ligation_barcode_file1, "rb")
    ligation_barcode_list1 = pickle.load(barcodes)
    barcodes.close()

    barcodes = open(ligation_barcode_file2, "rb")
    ligation_barcode_list2 = pickle.load(barcodes)
    barcodes.close()
    ...
    func = partial(gRNA_cell_UMI_ident, input_folder = input_folder, ligation_barcode_list1=ligation_barcode_list1, 
                   inner_i7_bc_list=inner_i7_bc_list, ligation_barcode_list2=ligation_barcode_list2, 
...
if __name__ == "__main__":
    ...
    inner_i7_bc_file = sys.argv[4]
    ligation_barcode_file1 = sys.argv[5]
    ligation_barcode_file2 = sys.argv[6]
    gRNA_correction_file = sys.argv[7]
    gRNA_annotation_df = sys.argv[8]
    min_UMI_threshold = int(sys.argv[9])
    core = int(sys.argv[10])
    gRNA_UMI_ident_parallel(input_folder, sampleID, output_folder, ligation_barcode_file1,
                            inner_i7_bc_file, ligation_barcode_file2, gRNA_correction_file, 
                            gRNA_annotation_df, min_UMI_threshold, core)
