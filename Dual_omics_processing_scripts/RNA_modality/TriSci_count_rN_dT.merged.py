# Modified by Andras Sziraki, 03/31/2021: Because randomN RT reads are not necessarily mapping to the gene with closer 3 end if the read maps to multiple genes, I check if the RT barcode is a randomN barcode and if yes, these multi-gene mapping reads are labelled as ambigous instead of trying to assign this read.
...
def sciRNAseq_count(sample, input_folder, exons, genes, gene_end, gene_annotat, sample_ID):
    ...
    ###in TriSci, dT and rN are not separated anymore
        ...
        if len(gene_id_intersect) == 1:
            ...
            if len(gene_id_combine) == 1:
                ...
                if len(gene_id_intersect) == 1:
                    ...
                    gene_id = find_nearest_gene(alnmt.iv.end_d, gene_id_intersect, gene_end, RT_barcode) + "_intron"
                    counts[gene_id] += 1
                    ...
                    if len(gene_id_combine) == 1:
                        ...
                        gene_id = find_nearest_gene(alnmt.iv.end_d, gene_id_combine, gene_end, RT_barcode) + "_intron"
                        counts[gene_id] += 1
