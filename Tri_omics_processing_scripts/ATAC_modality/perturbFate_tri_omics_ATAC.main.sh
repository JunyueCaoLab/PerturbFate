...
for sample in $(cat $sample_ID); do echo aligning $sample; STAR --runThreadN $core --outSAMstrandField intronMotif --genomeDir $index --readFilesCommand zcat --readFilesIn $input_folder/$sample*R1*.gz --outFileNamePrefix $STAR_output_folder/$sample --genomeLoad LoadAndKeep --outFilterMultimapNmax 1; done
